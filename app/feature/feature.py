...
class Feature_MS2(Feature):
    ...
    :type rt_accuracy: float, optional
    """
    def __init__(self, feature_id, data_dict, mass_is_neutral = False, POSMODE = True, precursor_mass_accuracy = 10, fragment_mass_accuracy = 0.02, rt_accuracy = 0.2):
        super(Feature_MS2, self).__init__(feature_id, data_dict['MASS'], data_dict['RT'], precursor_mass_accuracy, rt_accuracy)
...
class MS2_Spectrum:
    ...
    def standardize_spectrum(self):
        ...
        self.spectrum_df = pd.DataFrame({'FRAGMENT_MASS':self.reduced_fragment_mass, 'INTENSITY':self.normalized_signal})
    ...
    def align(self, other_spectrum, frag_mass_accuracy = 0.2, suffixes = ['_self', '_other']):
        ...
        """        
        df_self = pd.DataFrame({'bin_mass': list(map(lambda x: round(x, 0), self.reduced_fragment_mass)),
                            'frag_mass': self.reduced_fragment_mass,
                            'intensity': self.normalized_signal})
        
        df_other = pd.DataFrame({'bin_mass': list(map(lambda x: round(x, 0), other_spectrum.reduced_fragment_mass)),
        ...
        if frag_mass_accuracy < 1:
            ...
            merged_df['mass_match'] = np.where(merged_df['mass_delta'] / merged_df['frag_mass'+suffixes[0]] * 1000000 < frag_mass_accuracy, 1, 0)
        ...
        aligned_df = merged_df[(~merged_df.duplicated(subset='frag_mass'+suffixes[0], keep='first')) &
                                (~merged_df.duplicated(subset='frag_mass'+suffixes[1], keep='first'))].copy()
