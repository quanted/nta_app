...
def connect_to_mongoDB(address):
    ...
    mongo.nta_runs.Collection.create_index(
        [("date", pymongo.DESCENDING)], expireAfterSeconds=86400
    )
...
def reduced_file(df_in):
    ...
    to_drop = [
        ...
        for item in sublist
        if (len(sublist) > 1) & (not any(x in item for x in keeps_str))
    ]
    to_drop.extend(
        df.columns[
            (df.columns.str.contains(pat="CV_|N_Abun_|Median_|STD_") == True)
        ].tolist()
    )
    to_drop.extend(
        df.columns[
...
def response_log_wrapper(api_name: str):
    def api_log_decorator(request_func):
        def wrapper(*args, **kwargs):
            ...
            logger.info(
                f"Response: {response}   Run time: {time.perf_counter() - start_time}"
            )
            return response
...
def api_search_masses(masses, accuracy, jobid="00000"):
    input_json = json.dumps(
        {"search_by": "mass", "query": masses, "accuracy": accuracy}
    )  # assumes ppm
...
def api_search_masses_batch(masses, accuracy, batchsize=50, jobid="00000"):
    ...
    while i < n_masses:
        ...
        dsstox_search_json = io.StringIO(
            json.dumps(response.json()["results"])
        )  # can be an empty string if no hits
        if i == 0:
...
def batch_search_hcd(dtxsid_list, batchsize=200):
    ...
    for i in range(0, len(dtxsid_list), batchsize):
        logger.info(
            f"HCD Query: {i//batchsize} of {len(dtxsid_list)//batchsize} batches"
        )
        ...
        for chemical in chem_data_list:
            ...
            for data in chemical["scores"]:
                result_dict[chemical_id][f'{data["hazardName"]}_score'] = data[
                    "finalScore"
                ]
...
def format_tracer_file(df_in):
    ...
    mass_diff = (
        (df["Observed_Mass"] - df["Monoisotopic_Mass"]) / df["Monoisotopic_Mass"]
    ) * 1000000
