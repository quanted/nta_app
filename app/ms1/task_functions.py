...
def clean_features(df_in, controls, tracer_df=False):
    ...
    an additional dataframe (docs). Sample-level detection counts are also calculated -- AC & TMF 12/11/23
    """
    ...
    for mean, Std, median, N in zip(
        Mean_MB, Std_MB, Median_Blanks, Replicate_Percent_MB
    ):
    ...
    docs[Mean_Samples] = np.where(
        cv_not_met & cell_empty & ~cell_empty_df, "CV", docs[Mean_Samples]
    )
    docs[Mean_Samples] = np.where(
        cv_not_met & ~cell_empty & ~cell_empty_df,
    ...
    df["BlkStd_cutoff"] = df["BlkStd_cutoff"].fillna(
        df[Mean_MB[0]]
    )  # In the case of a single blank replicate, the previous calculation is an empty value as it cannot calculate Std dev; replace with mean value
    ...
    docs[Mean_Samples] = np.where(
        ~MDL_sample_mask & cell_empty & ~cell_empty_df, "ND", docs[Mean_Samples]
    )
    docs[Mean_Samples] = np.where(
        ~MDL_sample_mask & ~cell_empty & ~cell_empty_df,
    ...
    docs["Feature_removed"] = np.where(
        (df[Replicate_Percent_MB[0]] != 0)
        & (df[Mean_Samples].max(axis=1, skipna=True) < df["BlkStd_cutoff"]),
        "BLK",
        "",
    )
    # Features dropped because all samples are below CV threshold
    docs["Feature_removed"] = np.where(
        (df[CV_Samples] > controls[1]).all(axis=1), "CV", docs["Feature_removed"]
    )
    # Features dropped because all samples are below replicate threshold
    docs["Feature_removed"] = np.where(
        (df[Replicate_Percent_Samples] < controls[0]).all(axis=1),
        "R",
    ...
    docs["Feature_removed"] = np.where(
        ((df[Replicate_Percent_MB[0]] == 0) & (df[Mean_Samples].count(axis=1) < 1)),
        "CV/R",
        docs["Feature_removed"],
    )
    ...
    df_flagged.drop(
        df_flagged[
            (df_flagged[Replicate_Percent_Samples] < controls[0]).all(axis=1)
        ].index,
        inplace=True,
    )
    ...
    df = df[
        ((df[Replicate_Percent_MB[0]] == 0) & (df[Mean_Samples].count(axis=1) > 0))
        | (df[Mean_Samples].max(axis=1, skipna=True) > df["BlkStd_cutoff"])
    ]
    df_flagged = df_flagged[
        (
            (df_flagged[Replicate_Percent_MB[0]] == 0)
            & (df_flagged[Mean_Samples].count(axis=1) > 0)
        )
        | (
            df_flagged[Mean_Samples].max(axis=1, skipna=True)
