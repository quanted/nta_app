...
def cfmid_response_to_dict(response, mass, mode):
    ...
    with io.StringIO(json.dumps(response["results"])) as cfmid_search_json:
        ...
        formated_data_dict = (
            cfmid_search_df.groupby(["DTXCID", "FORMULA", "MASS"])[["ENERGY", "FRAG_MASS", "FRAG_INTENSITY"]]
            .apply(lambda x: x.groupby("ENERGY")[["FRAG_MASS", "FRAG_INTENSITY"]].apply(lambda x: x.to_dict("list")))
            .to_dict("index")
        )
...
def cfmid_response_to_spectra(cfmid_response_dict):
    if cfmid_response_dict["data"]:
        for cfmid_spectra in cfmid_response_dict["data"]:
            cfmid_response_dict["data"][cfmid_spectra]["energy0"] = MS2_Spectrum(
                cfmid_response_dict["data"][cfmid_spectra]["energy0"]["FRAG_MASS"],
                cfmid_response_dict["data"][cfmid_spectra]["energy0"]["FRAG_INTENSITY"],
            )
            cfmid_response_dict["data"][cfmid_spectra]["energy1"] = MS2_Spectrum(
                cfmid_response_dict["data"][cfmid_spectra]["energy1"]["FRAG_MASS"],
                cfmid_response_dict["data"][cfmid_spectra]["energy1"]["FRAG_INTENSITY"],
            )
            cfmid_response_dict["data"][cfmid_spectra]["energy2"] = MS2_Spectrum(
                cfmid_response_dict["data"][cfmid_spectra]["energy2"]["FRAG_MASS"],
